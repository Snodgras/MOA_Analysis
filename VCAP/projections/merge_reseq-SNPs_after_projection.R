...
cross.list <- system("ls -d /work/LAS/mhufford-lab/snodgras/projections/GBS-output/tmp/B73* | xargs -n 1 basename",
                     intern = TRUE)
...
for (cross in cross.list) {
  ...
  filename.after.proj <- list.files(path = folder.after.proj,
                                    pattern = paste0(cross, ".best-markers.chr", chr,".projected.hmp.txt"),
                                    full.names = TRUE)
  ...
  all.svs.empty.hmp.file <- list.files(path = folder.after.proj,
                                       pattern = paste0(".empty.chr-", chr,".hmp.txt"),
                                       full.names = TRUE)
