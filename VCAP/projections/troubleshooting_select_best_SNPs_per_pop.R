...
cross <- "B73xB97"
data.filename <- "Desktop/MOA/VCAP/projections/NAM_rils_SNPs.B73xB97.not-imputed.hmp.txt"
reseq.parents.filename <- "Desktop/MOA/VCAP/projections/NAM_gbs_B73xB97.alignoverlay.noslashes.hmp.txt"
folder.for.plots <- "Desktop/MOA/VCAP/projections/localtestplots/"
...
gbs.filter <- c() #empty vector
for (chr in 1:10) { #for each of the 10 chromosomes
  chr.filter <- geno.data.infile[which(geno.data.infile[, "chrom"] == chr), "pos"] %in% # T/F for each row if that SNP occurs in the overlay file
    reseq.parents.infile[which(reseq.parents.infile[, "chrom"] == chr), "pos"]
  
  gbs.filter <- append(gbs.filter, chr.filter) #make a vector of all of the T/F across all chromosomes
}
...
# keep heterozygous, homozygous polymorphic markers between parents
...
window.test.infile<-tail(geno.data.infile.poly.top, n = 1)
for(chr in 1:10){
  temp<-head(geno.data.infile.poly.top[which(geno.data.infile.poly.top[,"chrom"] == chr), ], n = 100)
  window.test.infile<-rbind(window.test.infile,temp)
}
###TROUBLE SHOOTING THE WINDOW STEP###
window.test.infile.window <- foreach(chr=1:10, .combine = rbind) %dopar% {
  
  # subset by chromsome
  chr.poly.window <- window.test.infile[which(window.test.infile[, "chrom"] == chr), ]
      ...
      while (window.stop <= NROW(chr.poly.window)) {
        ...
        window.calls <- apply(window, MARGIN = 1, FUN = function(genotypes) {
          if (genotypes[3] == "NN") {
            ...
            #THIS STEP IS THROWING A WARNING
...
#To show Allele frequency does center on 0.5
#ggplot(as.data.frame(af.filter), aes(x=af.filter))+geom_histogram(binwidth = 0.1)
