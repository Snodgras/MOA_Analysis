...
knitr::opts_knit$set(root.dir = "JeffsTest/bQTL_strict_Oct2023/")
```
...
```{r}
bqtl_K<-make_K("bQTL.SNPs.bed_CentIBS_K") 
sbqtl_K<-svd(bqtl_K)
...
cbqtl_K = sqrt(sbqtl_K$d) * t(sbqtl_K$u) # chol(moa_K)
...
U = matrix(0,nrow = nrow(cbqtl_K),ncol = 4)
U[,1] = crossprod(cbqtl_K,rnorm(nrow(cbqtl_K)))
U[,2] = crossprod(cback_K,rnorm(nrow(cback_K)))
U[,3] = crossprod(crest_K,rnorm(nrow(crest_K)))
U[,4] = rnorm(nrow(cbqtl_K))
...
write_tsv(Sim_Traits, paste("Simulated_Phenotypes",".","bQTLstrict",".txt",sep = ""))
```
